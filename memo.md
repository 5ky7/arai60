...
- 2つに限らず複数の値の足し算の関数を作って，そのインスタンスとして解いてみる（下のコード）．
  - addNumbersの引数が参照渡しなのは議論の余地あり（cf. 「ライブラリだったとして引数に入れたものが勝手に変更されたら驚く」を見て確かになぁと思うなど）だが，コピー渡しにすると大規模化したとき遅くなるというデメリットもある．状況を見て判断，今回は「大元のaddTwoNumbersのl1,l2が変更されなければOK」として参照渡しでいいかなと（あくまでaddNumbersはaddTwoNumbersのためにあると解釈）．
  - `addTwoNumbers`と`addNumbers`の返り値とか引数が合ってないのは気持ち悪いなと思って修正したところ（下の下のコード），なぜか実行時間が1ms以下から7ms程度まで増加
```c++
#include <vector>
using std::vector;
